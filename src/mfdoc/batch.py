...
_RECONCILIATION_INSTRUCTIONS = (
    ...
    "below; every sentence you write must carry at least one citation copied "
    "from one of them. A sentence that genuinely generalizes across more "
    "than one chunk (a whole-module claim no single excerpt supports on its "
    "own) may carry more than one citation, comma-separated -- one for each "
    "chunk excerpt it draws from -- but every citation in that list must "
    "still be copied verbatim, character for character, from the excerpts "
    "below; never invent a new one, even one that only resembles or "
    "interpolates between citations you were given. Where excerpts "
    "genuinely conflict, "
    "prefer the more specific or more heavily-cited statement and note the "
    "discrepancy as an `(unresolved)` item rather than silently picking "
    "one.\n\n"
