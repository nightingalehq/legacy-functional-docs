...
class _TransientCLIError(RuntimeError):
    """A `claude -p` failure this caller considers worth retrying (issue
    #79's consistency review comment: AnthropicCaller/VertexCaller retry
    transient errors, this caller should too). Deliberately narrow -- only
    a subprocess timeout, the clearest "this looked like a network/load
    blip, not a real problem" signal available from an opaque CLI
    subprocess. A non-zero exit, unparseable JSON, or an explicit
    `is_error` result are left as plain (non-retryable) `RuntimeError`:
    those usually mean something genuinely wrong (a bad prompt, a broken
    install, an auth failure) that a retry would just repeat identically,
    burning another `--claude-code-timeout`-long wait for no chance of a
    different outcome."""
    ...
    def __call__(self, prompt: str) -> ModelResponse:
        def do_call() -> ModelResponse:
            ...
            try:
                ...
                raise _TransientCLIError(f"`claude -p` timed out after {self.timeout}s") from exc
        ...
        return call_with_retry(
            do_call, lambda exc: isinstance(exc, _TransientCLIError),
